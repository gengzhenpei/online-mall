...
//查找sku的价格
  ...
  async detailAction() {
    const goodsId = this.get('id');
    console.log(goodsId);
    const collage = await this.model('collage').where({goodsid:goodsId,is_abled:1}).select()
    const model = this.model('goods');
    ...
    // const attribute = await this.model('goods_attribute').field('shop_goods_attribute.value, shop_attribute.name').join('shop_attribute ON shop_goods_attribute.attribute_id=shop_attribute.id').order({'shop_goods_attribute.id': 'asc'}).where({'shop_goods_attribute.goods_id': goodsId}).select();
    ...
    const hotComment = await this.model('comment').where({goods_id: goodsId}).find();
    let commentInfo = {};
    if (!think.isEmpty(hotComment)) {
      ...
      commentInfo = {
        ...
        add_time: think.datetime(new Date(hotComment.add_time * 1)),
